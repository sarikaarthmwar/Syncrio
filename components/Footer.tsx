...
const columns = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: "Intelligent Automation", href: "/services/process-automation" },
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 py-16 sm:grid-cols-2 lg:grid-cols-5 lg:px-8">
        <div className="sm:col-span-2 lg:col-span-2">
          <a href="#home" className="flex items-center gap-2.5 text-lg font-semibold text-white">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-secondary to-accent font-bold text-white">S</span>
            Syncrio
          </a>
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-slate-400">
            AI-Powered Enterprise Delivery &amp; Transformation. We help SaaS and enterprise organizations deliver faster, smarter, and more predictably.
          </p>
          <div className="mt-5 flex gap-3">
            {socialLinks.map(({ Icon, href, label }) => (
              <a key={label} href={href} aria-label={label} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noopener noreferrer" : undefined} className="grid h-9 w-9 place-items-center rounded-full border border-white/10 text-slate-300 transition-colors hover:border-white/30 hover:text-white">
                <Icon size={16} />
              </a>
            ))}
          </div>
        ...
        {columns.map((col) => (
          <div key={col.title}>
            ...
            <ul className="mt-4 space-y-2.5 text-sm">
              {col.links.map((l) => (
                <li key={l.label}>
                  <a href={l.href} className="text-slate-400 transition-colors hover:text-white">{l.label}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div>
          ...
          <form className="mt-3 flex gap-2">
            <input type="email" placeholder="you@company.com" aria-label="Email address" className="w-full rounded-full border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:border-secondary focus:outline-none" />
          </form>
        </div>
      </div>
